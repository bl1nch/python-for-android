...
class Python3Recipe(TargetPythonRecipe):
    ...
    def _libpython(self):
        ...
        return "libpython{link_version}.so".format(link_version=self.link_version)
    ...
    def link_version(self):
        ...
        if major == "3" and int(minor) < 8:
            flags += "m"
        return "{major}.{minor}{flags}".format(major=major, minor=minor, flags=flags)
    ...
    def get_recipe_env(self, arch=None, with_flags_in_cc=True):
        ...
        if shutil.which("lld") is not None:
            ...
            warning(
                "lld not found, linking without it. "
    ...
    def build_arch(self, arch):
        ...
        android_build = (
            sh.Command(join(recipe_build_dir, "config.guess"))()
            .stdout.strip()
            .decode("utf-8")
        )
